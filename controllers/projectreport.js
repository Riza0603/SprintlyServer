...
export const generateReport = async (req, res) => {
  try {
    ...
    const teammates = await UserModel.find({ projects: project.pname });

    // Extract name and role from teammates for the PDF generation
    const teamMembers = teammates.map((member) => ({
      name: member.name, 
    ...
    //Team Members & Roles
    doc.moveDown(0.5);
    doc.fontSize(18).fillColor("#1F2D3D").text("Team Members & Roles", { underline: true }).moveDown(0.5);
    ...
    doc.text("Role", 240, teamTableTop + 5);
